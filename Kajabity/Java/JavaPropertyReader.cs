...
namespace Kajabity.Tools.Java
{
	...
	public class JavaPropertyReader
	{
		...
		{ "", "ACTION_add_to_key", "ACTION_add_to_value", "ACTION_store_property", "ACTION_escape", "ACTION_ignore" };
			...
			do
			{
				...
				for( int s = 0; s < states[ state ].Length; s += 3 )
				{
					if( matches( states[ state ][ s ], ch ) )
					{
						...
						matched = true;
                        //Console.WriteLine(actionNames[states[state][s + 2]] + ", " + states[state][s + 2] + ", " + ch + (ch > 20 ? " (" + (char)ch + ")" : ""));
		...
		private void doAction( int action, int ch )
		{
			switch( action )
			{
				...
				case ACTION_store_property:
					Console.WriteLine( "ACTION_store_property: " + keyBuilder.ToString() + "=" + valueBuilder.ToString() );
                    hashtable.Remove(keyBuilder.ToString());
